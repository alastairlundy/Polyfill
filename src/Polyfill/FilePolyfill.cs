...
#if NET7_0_OR_GREATER || FeatureRuntimeInformation
    public static UnixFileMode GetUnixFileMode(string file) =>
#if NET7_0_OR_GREATER
        return File.GetUnixFileMode(file);
#else
        return GetUnixFileModeFallback(file);
#endif

    #if !NET7_0_OR_GREATER
    private static UnixFileMode GetUnixFileModeFallback(string file)
{
        ...
        string output = RunProcess("{Path.DirectorySeparatorChar}usr"+
"{Path.DirectorySeparatorChar}bin{Path.DirectorySeparatorChar}stat", $"-c %a {file}")

        bool isValid = float.TryParse(output, out float octalFloat);
